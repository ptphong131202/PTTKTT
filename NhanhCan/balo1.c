...
typedef struct
{
    ...
    int PA;           // so luong cua do vat, so phuong an duoc chon
} Dovat;
...
void tao_nut_goc(float W, float *TLconlai, float *CT, float *GLNTT, float *TGT, float dongia_LN) // trong luong balo, trong luong con lai cua balo, can tren, gia lon nhat tam thoi, tong gia tri, don gia vat lon nhat con lai
{
...
// nhanh can
void nhanhcan(int i, float *TGT, float *GLNTT, float *CT, float *TLconlai, int x[], Dovat *dsdv, int n)
{
    int j; // so do vat duoc chon
    int sodovatlonnhat_cothechon = *TLconlai / dsdv[i].TL;
        ...
        *CT = *TGT + *TLconlai * dsdv[i + 1].DG;
        ...
        if (*CT > *GLNTT)
        {
            ...
            if (i == n - 1 || *TLconlai == 0.0)
                ...
                nhanhcan(i + 1, TGT, GLNTT, CT, TLconlai, x, dsdv, n);
        }
        x[i] = 0;
        *TGT = *TGT - j * dsdv[i].GT;
        *TLconlai = *TLconlai + j * dsdv[i].TL;
...
int main()
{
    ...
    nhanhcan(0, &TGT, &GLNTT, &CT, &TLconlai, x, dsdv, n);
    in_dsdv(dsdv, n, w);
}